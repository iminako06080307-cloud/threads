...
思い込み論破 / 維持のための習慣シェア / マインドセット / ○○VS○○比較 / 実は太りやすい食品 /
太りにくい1日の食事例 / **痩せレシピ** / お客様の声・実績紹介 の8種。各フォーマットに相性の良い
スタイル(`bestStyle`)を設定でき、管理画面に「◎このスタイルと好相性」と表示されます。

既定は **みち風（等身大・ズボラ系）スタイル**（`DEFAULT_STYLE_ID` / `DEFAULT_FORMAT_ID`）です。
書き出し「成果+対象+こちらーーー！！！」、【 】見出し、①②③手順、🙏🔥❤️‍🔥 の絵文字など、
みち式の型を再現します（減量の数字は捏造せず、体型維持の正直な表現に置き換え）。
...
## 無料勉強会・サポート生募集 (SEMINAR フォーマット)

サポート生のビフォアフ → 方法を小出し → 無料勉強会に募集（席限定）する導線用の型です。

1. フォーマットで **「無料勉強会・サポート生募集」** を選ぶ
2. サポート生さんの**実際の取り組み・変化**を「素材」欄に貼る（本人同意が前提）
3. **ビフォアフ写真**を「画像URL」に入れる
4. 生成すると、末尾に打消し表示＋**参加先URL（`▼参加はこちら▼`）**が自動で付きます

参加先は `SEMINAR_CTA_URL`（未設定なら `LINE_CTA_URL` にフォールバック）。
「席に限りがある」等の記載は**事実の範囲**で（虚偽の緊急性はNG／景品表示法）。
